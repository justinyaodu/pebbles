...
with open('singlethread10.txt') as f:
    lines = f.readlines()
    pebbles_times_st_random10 = [sum([float(line) for line in lines])] * 8

with open('multithread10.txt') as f:
    lines = f.readlines()
    pebbles_times_mt_random10 = [sum([float(line) for line in lines])] * 8

with open('gpu_version10.txt') as f:
    lines = f.readlines()
    pebbles_times_gpu_random10 = [sum([float(line) for line in lines if not line.startswith("Synthesizer")])] * 8
...
ax = plt.gca()
ax.set_ylim([0, 10])
plt.plot(cvc4_times,'o',markersize=2, label="cvc4")
plt.plot(pebbles_times,'o',markersize=2, label="pebbles")
plt.ylabel("Execution time in seconds")
plt.xlabel("SyGuS CrCi problems")
plt.title("Comparison of CVC4 1.8 and Pebbles")
plt.xticks([], []) # turn off x-ticks
leg = plt.legend(loc='upper left')
plt.savefig('cvc4_versus_pebbles_linear.png')
plt.show()
...
plt.xticks([], []) # turn off x-ticks
leg = plt.legend(loc='upper left')
plt.savefig('cvc4_versus_pebbles_log.png')
plt.show()

# plt.yscale('log')
# plt.plot(cvc4_times_solved,'o',markersize=2, label="cvc4")
# plt.plot(pebbles_times_solved,'o',markersize=2, label="pebbles")
# print(pebbles_times[len(pebbles_times)//2+2],cvc4_times[len(pebbles_times)//2+2])
# plt.ylabel("Execution time in seconds")
...
#plt.yscale('log')
# plt.plot(pebbles_times_st_random10,'-',linewidth=1, markersize=2.5, label="singlethread")
# plt.plot(threadTimes,'o-',linewidth=1, markersize=2.5, label="multithread")
# plt.plot(pebbles_times_gpu_random10,'-',linewidth=1, markersize=2.5, label="gpu")
...
# plt.xticks(range(8), range(1,9)) # turn off x-ticks
